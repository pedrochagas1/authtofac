...
require 'validarUsuario.php';
    ...
    if($_SESSION['user'] = efetuarLogin($login, $senha)){
        ...
        try {
            ...
            $mail->Username   = 'contatoplsc@gmail.com';         //SMTP username
            $mail->Password   = 'kdudvutvdnjnyrcn';             //SMTP password gerado exclusivo para este envio, não é a mesma senha do email.
            $mail->Port       = 587;        //TCP port to connect to; use 587 if you have set `SMTPSecure = 

            //Recipients
            $mail->setFrom('contatoplsc@gmail.com', 'Sistema de Login');
            $mail->addAddress('contatoplsc@gmail.com', ''.$login.'');    //Add a recipient
            $mail->addReplyTo('contatoplsc@gmail.com', 'Responda para:');
            ...
            $mail->send();
            $query = "UPDATE autenticacao SET codigo = '$OTP', created_at = '10:33' WHERE id_user = 14 ";
